...
{
    // We will get back a list of post which is a json array of post json object. To tell retrofit what to do we have to annotate this method. As this is get
    // request so we will annotate this method using GET annotation. In GET annotation we will pass the 'posts' which represents relative url as base url
    // is handled in the RetrofitActivity.java. Instead of relative URL you can also pass the whole URl int GET() annotation and in this case it will
    // replace the base URL defined while creating the instance of the Retrofit. // While defining the base URL, backslash ( / ) is temporary means if not
    // defined here then it have to put at the start of each relative URL ( i.e instead of "posts" write "/posts" ).
    ...
    // Query annotation is used for query params. Retrofit will automatically add the question before the first query variable to indicate the start of the
    // query params ( url : "https://.....?userid=2&_sort=id... ). Again Retrofit will add ampersand (&) sign b/w each query parameter variables.
    @GET("posts")
    ...
    // Put request will replace the whole the post whose id is mentioned. Extra we can add header also as we did below.
    ...
    // Where as patch request will only update the those fields whose key-value is passed in the body ( or whose value is passed as null ).
    @PATCH("posts/{id}")
